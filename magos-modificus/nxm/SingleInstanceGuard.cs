...
/// any other process remains, another Magos is running.</para>
/// <para>
/// <b>Why process enumeration, not a pipe probe.</b> Process enumeration directly
/// answers "is another Magos already running?" without the startup tax a
    ...
    public void EnsureOnlyInstance(string ipcPipeName)
    {
        ...
        if (others.Length > 0)
        {
            _logger.LogInformation(
                "Single-instance check found another Magos process (pid {Pid}, name '{Name}').",
                others[0], name);
            throw new NxmSingleInstanceException(
                ipcPipeName,
                new InvalidOperationException(
                    $"Another Magos process (pid {others[0]}) with name '{name}' is already running."));
        }
    }
