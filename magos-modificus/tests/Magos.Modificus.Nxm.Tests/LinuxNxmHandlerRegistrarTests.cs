...
public sealed class LinuxNxmHandlerRegistrarTests
{
    ...
    public void Register_writes_desktop_file_with_expected_content()
    {
        ...
        try
        {
            var registrar = new LinuxNxmHandlerRegistrar(
                "/opt/magos/Magos.NxmHandler",
                NullLogger<LinuxNxmHandlerRegistrar>.Instance,
                applicationsDir: dir,
                runXdg: _ => (0, "magos-nxm-handler.desktop\n"));
            ...
            Assert.Contains("Name=Magos NXM Handler", content);
            Assert.Contains("Exec=\"/opt/magos/Magos.NxmHandler\" %u", content);
    ...
    private static string CreateTempApplicationsDir() =>
        Path.Combine(Path.GetTempPath(), "magos-nxm-test-" + Guid.NewGuid().ToString("N"));
