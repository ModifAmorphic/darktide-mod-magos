...
/// delivered, exit 0, Magos not launched), the cold-start path (pipe refuses,
/// Magos launched, retry connects, URL delivered, exit 0), the cold-start
/// timeout (Magos launched, retries exhausted, non-zero exit), the no-URL-arg
/// case (non-zero exit, Magos not launched), and the multi-arg case (first
/// non-flag used as the URL).
/// </summary>
/// <remarks>
/// Every external dependency is faked: <c>pipeConnect</c> returns a capturing
/// in-memory stream, <c>launchMagosFactory</c> returns a marker ProcessStartInfo
...
public sealed class NxmHandlerRelayTests
{
    ...
    [Fact]
    public async Task Hot_path_connects_and_delivers_without_launching_magos()
    {
        ...
        var exit = await NxmHandlerRelay.RunAsync(
            ...
            launchMagosFactory: () => { launched = true; return Marker(); },
            retryInterval: FastRetry,
            retryTimeout: FastTimeout);

        Assert.Equal(0, exit);
        Assert.False(launched, "Magos must NOT be launched on the hot path.");
        Assert.Single(pipe.Delivered);
        Assert.Equal(url, pipe.Delivered[0]);
    }

    [Fact]
    public async Task Cold_path_launches_magos_then_delivers_on_retry()
    {
        ...
        Assert.Equal(0, exit);
        Assert.True(launched, "Magos must be launched on the cold path.");
        Assert.Single(pipe.Delivered);
        Assert.Equal(url, pipe.Delivered[0]);
    }
    ...
    public async Task Cold_path_timeout_exits_nonzero_after_launching()
    {
        ...
        Assert.True(launched, "Magos must be launched even on the timeout path.");
    }
    ...
    public async Task No_url_arg_exits_nonzero_without_launching()
    {
        ...
        Assert.NotEqual(0, exit);
        Assert.False(launched, "Magos must NOT be launched when no URL is provided.");
    ...
    [Fact]
    public void ResolveMagosMainExe_throws_when_sibling_exe_is_missing()
    {
        // A fresh empty base dir: no sibling Magos.Modificus(.exe) lives here,
        // so the existence check must throw before any ProcessStartInfo is built.
        var baseDir = Path.Combine(Path.GetTempPath(), "magos-nxm-relay-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(baseDir);
        try
        {
            var ex = Assert.Throws<MagosMainExeNotFoundException>(
                () => NxmHandlerRelay.ResolveMagosMainExe(baseDir));
            ...
            var expectedName = OperatingSystem.IsWindows() ? "Magos.Modificus.exe" : "Magos.Modificus";
