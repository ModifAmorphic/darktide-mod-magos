...
/// <summary>
/// Pinned to a specific release. Two pins share only when their version strings
/// match exactly (raw string equality, see <see cref="AllocationResolver"/>).
/// </summary>
/// <remarks>
/// <para>
/// <see cref="Version"/> is the **raw release tag** as authored by the source
/// (a GitHub release tag or a Nexus file version), stored verbatim. GitHub tags
/// + Nexus versions are arbitrary strings, not SemVer, so <see cref="string"/>
/// is the right type: there is no parsing or normalization at this layer. The
/// pin is exact: two pins share only when their version strings are identical.</para>
/// </remarks>
public sealed record PinnedPolicy : ModVersionPolicy
{
    public PinnedPolicy() { }

    public PinnedPolicy(string version)
    {
    ...
    /// <summary>
    /// The exact release this policy freezes to: a raw tag string (e.g.
    /// <c>"1.2"</c>, <c>"v2.0.1"</c>, <c>"1.0.0-beta"</c>). Compared by string
    /// equality for the share decision.
    /// </summary>
