...
/// <c>magos_launcher.exe</c> directly — no Proton, no path translation (native
...
internal sealed class WindowsLaunchStrategy : IPlatformLaunchStrategy
{
    ...
    /// <c>magos_launcher.exe</c>). Paths pass through verbatim — Windows needs no
    /// <c>Z:\</c> translation.
    /// </summary>
    /// <remarks>
    /// <c>--log-level</c> is intentionally NOT emitted: <c>MagosConfig.Logging.Level</c>
    /// is a Serilog level name for Magos's own log, but the Enginseer shell's level
