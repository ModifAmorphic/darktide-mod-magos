...
//!
//! Mirrors the validated POC engine (`poc/phase2-runtime-discovery/engine/pe.c`)
//! but is a fresh, safe-Rust implementation.
...
impl Section {
    ...
    /// POC's `scan_bound`: use `min(raw_size, virtual_size)` so we never walk
    /// past real bytes into loader zero-fill or absent on-disk tails.
...
impl Pe {
    ...
    pub fn from_mapped(image: &[u8]) -> Result<Self, PeError> {
        ...
        let opt_size = u16le(image, e_lfanew + fh::SIZE_OF_OPTIONAL_HEADER)
            .ok_or(PeError::OptHeaderOob)?;
        ...
        let image_base =
            u64le(image, opt_base + oh::IMAGE_BASE).ok_or(PeError::OptHeaderOob)?;
        let size_of_image =
...
pub fn map_from_file(file: &[u8]) -> Result<Vec<u8>, PeError> {
    ...
    let opt_size = u16le(file, e_lfanew + fh::SIZE_OF_OPTIONAL_HEADER)
        .ok_or(PeError::OptHeaderOob)? as usize;
    let num_sections = u16le(file, e_lfanew + fh::NUMBER_OF_SECTIONS)
        .ok_or(PeError::OptHeaderOob)? as usize;
