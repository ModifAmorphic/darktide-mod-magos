...
//!
//! Mirrors the POC engine's capstone configuration (`disasm.c`): Intel
//! syntax, detail off (we match on `mnemonic`/`op_str` exactly as the POC's
//! substring classifiers do).
...
/// disassembling many bodies (the POC opens one per call, but reusing avoids
/// re-initialising the engine ~50k times during a cluster scan).
...
impl Disassembler {
    pub fn new_x86_64_intel() -> Result<Self, capstone::Error> {
        ...
        let mut cs = Capstone::new()
            ...
            .build()?;
        // Match the POC: detail off (op_str is enough).
    ...
    /// 256 bytes (mirrors POC `dt_disasm_range`). Returns owned instructions.
    pub fn disasm_range(
        &mut self,
        image: &[u8],
        begin: u32,
...
/// the body terminator (`ret` / tail-`jmp` / `int3` padding). Mirrors the POC
/// matchers' body-size computation.
