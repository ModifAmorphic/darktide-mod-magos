...
//!
//! These signatures are ported from the POC's authoritative
//! `poc/phase5-dmf-bootstrap/test/disasm_check.c` (and `expected_addrs.h`),
//! with one deliberate change: **no hardcoded absolute addresses**. The POC
//! keyed some matchers on specific helper RVAs (`index2adr @ 0xc72be0`,
//! `lj_gc_check @ 0xc82fc0`, …) and on absolute cluster-range checks
//! (`0xc70000..=0xc8ffff`). Those are build-specific. Here, helper-call checks
//! are relaxed to "a direct call within the live LuaJIT cluster" and cluster
//! bounds are computed at runtime from the discovered cluster window. The
//! remaining self-contained features (tags, offsets, loop edges, magic
//! constants, call counts) are what the POC's own evidence shows to be unique
//! over the cluster — and uniqueness is asserted empirically by the oracle
//! Tier-2 test (`tests/oracle.rs`).

use crate::disasm::{body_end_rva, parse_branch_target, DecodedInsn, Disassembler, X86_INS_CALL,
                    X86_INS_JNE, X86_INS_JLE, X86_INS_LEA, X86_INS_MOVABS, X86_INS_RET,
                    X86_INS_TEST};
use crate::pe::Pe;
...
fn count_indirect_calls(insns: &[DecodedInsn]) -> usize {
    insns
        ...
        .filter(|i| {
            let o = i.op_str.trim();
            !(o.starts_with("0x") || (o.starts_with(|c: char| c.is_ascii_digit()) && !o.starts_with("0x")))
        })
        .count()
}
...
fn count_backward(insns: &[DecodedInsn], id: u32) -> usize {
    insns
        .iter()
        .filter(|i| i.id == id)
        .filter(|i| parse_branch_target(i).map(|t| t < i.address).unwrap_or(false))
        .count()
}
...
pub fn match_lua_atpanic(insns: &[DecodedInsn]) -> bool {
    ...
    let reads_glref = insns.iter().any(|i| {
        i.op_contains("[rcx + 8]") && (i.op_contains("r8") || i.op_contains("eax") || i.op_contains("rax"))
    });
...
/// `lua_pcall(L, nargs, nresults, errfunc)` — the POC's rich feature set
/// (production-spec / `disasm_check.c`).
pub fn match_lua_pcall(insns: &[DecodedInsn]) -> bool {
    ...
    reads_glref && reads_base && reads_top && reads_stack
        && test_r9 && jne && jle && lea_r9x8 && inc_r8 && shl_sub
        && calls.len() == 1
}
...
pub fn match_lua_setfield(insns: &[DecodedInsn], _c: Cluster) -> bool {
    ...
    // features above, which the POC's evidence shows are unique over the cluster.
...
pub fn match_lua_tolstring(insns: &[DecodedInsn]) -> bool {
    ...
    let len_null = insns.iter().any(|i| {
        i.id == X86_INS_TEST && (i.op_contains("rdi") || i.op_contains("r8"))
    });
...
pub fn match_lua_createtable(insns: &[DecodedInsn]) -> bool {
    ...
    let prologue = insns.iter().take(12).any(|i| {
        i.is_mnemonic("mov") && i.op_contains("[rcx + 8]")
    }) && insns.iter().take(12).any(|i| {
        i.is_mnemonic("mov") && i.op_contains("r8d")
    });
...
pub fn match_lua_type(insns: &[DecodedInsn]) -> bool {
    let magic = insns.iter().any(|i| {
        i.id == X86_INS_MOVABS && i.op_contains("0x75a0698042110")
    });
    let calls = direct_call_targets(insns);
    magic && calls.len() == 1
}
...
pub fn match_lua_tonumber(insns: &[DecodedInsn]) -> bool {
    let num_tag = has_const_in_op(insns, "0xfffeffff")
        || insns.iter().any(|i| i.is_mnemonic("cmp") && i.op_contains("-131073"));
    ...
    let movsd = insns.iter().any(|i| {
        i.is_mnemonic("movsd") && i.op_contains("xmm0") && i.op_contains("[rax]")
    });
    ...
    let prologue = insns.iter().take(10).any(|i| {
        i.is_mnemonic("mov") && i.op_str.trim() == "ebx, edx"
    }) && insns.iter().take(10).any(|i| {
        i.is_mnemonic("mov") && i.op_str.trim() == "rdi, rcx"
    });
...
pub fn match_lua_settop(insns: &[DecodedInsn]) -> bool {
    ...
    let idx_scale = insns
        ...
        && insns.iter().any(|i| {
            (i.is_mnemonic("shl") || i.is_mnemonic("sal")) && i.op_contains(", 3")
        });
...
pub fn match_lua_newstate_body(insns: &[DecodedInsn]) -> bool {
    ...
    let no_tvalue_tag = !has_const_in_op(insns, "0xfffffff")
        && !has_const_in_op(insns, "0xfffeffff");
...
/// POC `lua_load_wrapper_candidate` classifier; the one-large-callee shape
/// disambiguates it from the other `luaL_load*` wrappers in the cluster (which
/// the POC further pinned via the `lua_resource::bytecode` call-site context —
/// here uniqueness is asserted empirically by the oracle Tier-2 test).
pub fn match_lual_loadbuffer(insns: &[DecodedInsn], pe: &Pe, image: &[u8], _c: Cluster) -> bool {
    ...
    let calls = direct_call_targets(insns);
    let has_load_callee = calls.iter().any(|&t| {
        callee_body_size(pe, image, t as u32) > 8000 || t < 0x10_000
    });
