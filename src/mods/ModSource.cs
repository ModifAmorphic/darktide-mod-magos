...
/// container came from (Untracked / Nexus). Stored on
...
/// <c>untracked</c>/<c>nexus</c>, independent of assembly names), mirroring the
/// established <see cref="ModVersionPolicy"/> serialization.</para>
/// <para>
...
/// (unlike Nexus, whose identity is fully on the source). Container lookup for
/// an untracked source goes through
/// <see cref="IModRepository.FindUntrackedByName"/>; Nexus lookup goes through
/// <see cref="IModRepository.FindBySource"/>. This keeps the source hierarchy
/// faithful to the spec while still letting the import service dedup
