# UI (`Magos.Modificus.UI`): reference

> The Avalonia 12 front end of Magos Modificus. Owns the shell, profile
...
- The active id, restored from `IAppStateStore` at startup (straight into
  ...
  while Magos was closed) resolves to no selection in the shell and is
...
(`MagosComposition.StartRunningStatePolling`). The session's own logic (gate,
...
language) to the running app and persisting them to `MagosConfig`.
...
{
    public LocalizationService();   // over "Magos.Modificus.UI.Resources.Strings"
...
`Strings.resx` lives at `magos-modificus/ui/Resources/Strings.resx` under
the default namespace `Magos.Modificus.UI`.
...
2. **First Nexus auth `None` to configured transition** (gated by the
   persisted `MagosConfig.Nexus.DmfAuthPromptShown` flag so it fires once
...
- `TickInterval`: the periodic timer's fixed tick granularity (1 minute).
  ...
  (`MagosConfig.Nexus.AutoUpdateCheckIntervalMinutes`) is honored to this
...
The composition root is `magos-modificus/ui/MagosComposition.cs` (a static
...
`App.OnFrameworkInitializationCompleted` runs `MagosComposition.Build()`,
...
- **Magos libraries:** `config` (`MagosConfig`, `PreferencesConfig`,
...
`Magos.Modificus.UI.Tests` covers:
...
- [Magos Modificus architecture](../../architecture/MAGOS-MODIFICUS.md): the
  high-level tie-together (component model, the Enginseer contract Magos
...
- [config](config.md): `MagosConfig`, `PreferencesConfig`, `ThemeMode`,
  `NexusConfig`.
