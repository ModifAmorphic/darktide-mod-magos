...
- **`Magos.Modificus.Nxm`** (this library): URL types + parser, IPC framing, the
  Magos-side IPC server, the single-instance guard, the router + handler seams,
...
- **`Magos.NxmHandler`** (`magos-modificus/nxm-handler/`): the OS-registered
  ...
  Magos and retries the pipe until it comes up.
- **`Magos.Modificus.Nxm.Tests`** (xUnit): covers the parser, framing, IPC server
  resilience + single-instance guard, router, relay helper, the Linux registrar,
  and the `AddNxm` DI wiring.

The handler is deliberately dumb: it forwards the raw URL string. Magos owns URL
...
server exists). Process enumeration directly answers "is another Magos already
...
- **Fixed pipe name `Magos.Nxm`.** No per-user suffix: single-user gaming-app
...
- **Startup is two separate checks with two separate outcomes** (single-instance
  ...
  1. **Single-instance** via `SingleInstanceGuard.EnsureOnlyInstance` (process
     enumeration). If another Magos process is found, `Bind` throws
     `NxmSingleInstanceException`. This is fatal: it propagates out of
     `MagosComposition.Build` to `App.OnFrameworkInitializationCompleted`, which
...
2. Hot path (Magos running): try `pipeConnect`. On success, write the framed URL,
   close, return `0`.
3. Cold start (Magos not running): on refused connect, locate the sibling Magos
   exe via `launchMagosFactory`, start it detached (no args), retry `pipeConnect`
   every `retryInterval` (250ms) until success or `retryTimeout` (30s) elapses,
   then deliver and return `0`.
4. On timeout: log "Magos did not start within Ns" to stderr and return `3`.
...
`MagosMainExeNotFoundException` if it does not. The relay logs this to stderr and
...
- **`LinuxNxmHandlerRegistrar`** (`[SupportedOSPlatform("linux")]`): writes
  ...
  `xdg-mime default magos-nxm-handler.desktop x-scheme-handler/nxm`. The
...
1. `MagosComposition.Build()` calls `AddNxm()` during registration, then calls
...
<AppContext.BaseDirectory>/
  Magos.Modificus(.exe)            the main app (UI + composition root)
  Magos.NxmHandler(.exe)           the OS-registered scheme handler (native AOT)
  Magos.Modificus.Nxm.dll          shared library (router, server, framing, relay)
```

The handler exe and the main app ship as siblings. The handler resolves the main
exe by relative path (`NxmHandlerRelay.ResolveMagosMainExe`), and the OS
...
- **Magos not running (cold start):** the handler's connect is refused, so it
  ...
  exits. Magos starts normally; it has no `--nxm` arg and no cold-start branch.
...
```sh
dotnet publish magos-modificus/nxm-handler -c Release     # stripped native binary
```
