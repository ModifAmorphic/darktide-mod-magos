...
- **Flags:** `--game-binary <path>` (required) · `--mod-path <path>` (the mod
  ...
  (default `1361210`).
- **The mod root (`--mod-path`) is what Magos owns and writes:** DMF + user
  mods + `mods.lst` (the load-order file).
...
- **What Enginseer does NOT do (so Magos must):** load-order computation,
  dependency resolution, DMF acquisition, profile/staging management, platform
  plumbing on Linux.
...
- DMF appears as a protected first entry (locked first by dependency
  resolution; updateable).
