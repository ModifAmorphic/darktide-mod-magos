...
//!
//! All operate on an RVA-laid-out `image` and report RVAs. Mirrors the
//! validated POC engine (`poc/phase2-runtime-discovery/engine/scan.c`)
//! byte-for-byte (same opcodes, same offset math) but in safe Rust.
//!
...
/// (RIP-relative, no SIB). Target = insn_rva + 7 + disp32. Mirrors the POC
/// `find_lea_xrefs` exactly.
