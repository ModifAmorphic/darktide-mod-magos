 ...
 *   2. Call inject_and_resume(stub_target.exe, stub_shell.dll, timeout)
 *   3. Verify the stub process exits cleanly (exit code 0)
 *
...
void test_injection_stub(void) {
    ...
    /* Execute the full injection flow with a 30-second timeout. */
    int rc = inject_and_resume(stub_exe, stub_dll, 30000);
    ASSERT_EQ(0, rc);
