...
//! pattern cluster scan) → a complete [`AddressTable`] of all 16 target
//! functions.
//!
...
pub fn discover(image: &[u8]) -> Result<AddressTable, DiscoverError> {
    ...
    let mut dis = Disassembler::new_x86_64_intel().map_err(|e| DiscoverError::Disasm(e.to_string()))?;
...
pub fn discover_with(
    ...
    let preload_rva = string_rva(pe, image, b"_PRELOAD\x00")
        .ok_or(DiscoverError::Anchor("_PRELOAD"))?;
    ...
    let cluster = Cluster { lo, hi, preload_str_rva: preload_rva };
    ...
    let lua_pushcclosure =
        find_unique(dis, image, pe, &candidates, |i| patterns::match_lua_pushcclosure(i, cluster))
    ...
    // LuaEnvironment::init (the POC's proven method — a body signature is
    // ambiguous because many functions make indirect calls). The trace yields
...
/// reference the `stingray::lua_resource::bytecode` string (the bytecode-
...
/// inside `LuaEnvironment::init` (the POC's `dt_identify_lua_newstate`).
///
/// Recipe:
...
fn find_newstate_via_trace(
    ...
    let atpanic_idx = insns.iter().position(|i| {
        i.id == X86_INS_CALL && parse_branch_target(i) == Some(atpanic_rva as u64)
    })?;
