#!/usr/bin/env bash
# build.sh — Phase 4 mingw cross-compile of the dbghelp proxy DLL with
# the in-process discovery engine + Phase 3 lua_newstate capture +
# Phase 4 lua_pcall execution hook.
#
...
#                                         OFF so Phase 4 owns lua_pcall)
#   - Phase 4's src/                       (dllmain.c + inject.{c,h})
#
...
#   3 lua_pcall-candidate observers (Phase 4 owns lua_pcall now). This is
...
# ---- 5. Compile Phase 4 DLL sources ----------------------------------
echo "[build] compiling Phase 4 DLL sources (mingw)"
