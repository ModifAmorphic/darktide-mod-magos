#!/usr/bin/env bash
# install.sh — place the Phase 2b dbghelp.dll into the Darktide binaries dir.
#
# Same backup discipline as Phase 1: back up the game's original dbghelp.dll
# to dbghelp.dll.orig BEFORE overwriting, and refuse to clobber an existing
# backup (run uninstall.sh first to restore). Also cleans stale log/json.
set -euo pipefail
...
echo ""
echo "Look for the discovery summary line:"
echo "  grep 'discover summary' \"$GAME_DIR/darktide-poc.log\""
