#!/bin/sh
# Deterministic tests for scripts/uninstall.sh.
#
...
#
# Usage: sh scripts/tests/test-uninstall.sh
...
uninstaller="$scripts_dir/uninstall.sh"
...
# Near-match: same managed handler path but the wrong field code (%U capital).
# Exact-line matching must NOT treat it as AppImage-owned; the old substring
# match would have deleted it.
make_nxm_desktop_near_match_field_code() {
    mkdir -p "$T_APPS"
    printf '[Desktop Entry]\nType=Application\nName=Modificus Curator NXM Handler\nExec="%s" %%U\nNoDisplay=true\nMimeType=x-scheme-handler/nxm;\n' "$managed_handler_path" > "$nxm_desktop_path"
}
# Near-match: exact path + %u but trailing arguments on the Exec line, so the
# whole line differs from the registrar-emitted form.
make_nxm_desktop_near_match_extra_args() {
    mkdir -p "$T_APPS"
    printf '[Desktop Entry]\nType=Application\nName=Modificus Curator NXM Handler\nExec="%s" %%u --debug\nNoDisplay=true\nMimeType=x-scheme-handler/nxm;\n' "$managed_handler_path" > "$nxm_desktop_path"
}
# Unrelated desktop whose Exec points at a different handler but whose Comment
# line contains the managed handler path as a substring. The old substring
# match would have deleted it; exact-line matching preserves it.
make_nxm_desktop_substring_in_comment() {
    mkdir -p "$T_APPS"
    printf '[Desktop Entry]\nType=Application\nName=Other Handler\nComment=migrated from %s\nExec="/opt/other/handler" %%u\nNoDisplay=true\nMimeType=x-scheme-handler/nxm;\n' "$managed_handler_path" > "$nxm_desktop_path"
}
# Standalone-targeted Exec with the managed handler path appearing only as a
# substring in a Path field. The old substring match would have deleted it.
make_nxm_desktop_substring_in_path() {
    mkdir -p "$T_APPS" "$T_ROOT/app"
    : > "$T_ROOT/app/Modificus.Curator.NxmHandler"
    printf '[Desktop Entry]\nType=Application\nName=Modificus Curator NXM Handler\nPath=%s\nExec="%s" %%u\nNoDisplay=true\nMimeType=x-scheme-handler/nxm;\n' "$managed_handler_path" "$T_ROOT/app/Modificus.Curator.NxmHandler" > "$nxm_desktop_path"
}
make_user_data() {
...
# ===========================================================================
# Test 8b: default preserves near-match NXM desktops whose content contains
# the managed handler path but whose Exec line is not the exact registrar-
# emitted form. Proves exact full-line ownership (the prior substring match
# would have deleted each of these).
# ===========================================================================
# Wrong field code (%U instead of %u): path is present, line is not exact.
setup_env default-nxm-near-match-field-code
make_appimage
make_managed_appimage
make_nxm_desktop_near_match_field_code
expect_ok run_uninstall
...
# Trailing arguments on the Exec line: path + %u present, line is not exact.
setup_env default-nxm-near-match-extra-args
make_appimage
make_managed_appimage
make_nxm_desktop_near_match_extra_args
expect_ok run_uninstall
...
# Managed handler path appears only as a substring in Comment; Exec is foreign.
setup_env default-nxm-substring-in-comment
make_appimage
make_managed_appimage
make_nxm_desktop_substring_in_comment
expect_ok run_uninstall
...
# Standalone-targeted Exec with the managed path appearing only in a Path
# field (substring). The standalone handler must be left intact too.
setup_env default-nxm-substring-in-path
make_appimage
make_managed_appimage
make_nxm_desktop_substring_in_path
expect_ok run_uninstall
...
# Sanity: the positive exact AppImage-managed desktop IS still removed.
setup_env default-nxm-appimage-exact
make_appimage
make_managed_appimage
make_nxm_desktop_appimage
expect_ok run_uninstall
